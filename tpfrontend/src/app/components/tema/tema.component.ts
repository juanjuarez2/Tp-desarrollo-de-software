...
export class TemaComponent implements OnInit {
    ...
    if (this.temaForm.valid) {
      ...
      if (this.editing && this.currentTemaId) {
        ...
        this.temaService.update(this.currentTemaId, tema).subscribe(() => {
          this.getAllTemas();  // Actualizamos la lista de docentes
  ...
  deleteTema(id: number): void {
    this.temaService.delete(id).subscribe(() => {
      this.getAllTemas();  // Actualizamos la lista de docentes tras eliminar
    });
  }
  ...
  resetForm(): void {
    ...
    this.currentTemaId = null;  // Limpiamos el ID en edición
  }
